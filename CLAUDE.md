...
Read `PRD.md` for the original product requirements. Some UI sections (colors, overlay) are superseded by the current terminal aesthetic; the architecture sections are still canonical. Field names and the plugin API examples in §6 have been updated to match the live implementation (the `focus_min` → `focus_duration` drift from [M-11] is fixed).

## Audit history

- `AUDIT_REPORT.md` — initial bug / perf / security audit (v0.0).
- `AUDIT_V2.md` — second audit pass with `[ID]` markers in the source for every fix landed during Phase 6.5.
- `AUDIT_V3_RUNTIME.md` — **latest audit** (v0.1 ship-readiness gate + sandbox extensibility stress test). The sandbox expansion (interval authoring, render specs, prompts, full before-hook coverage, stats / preset API exposure) is the v0.1 → v0.2 work item; this CLAUDE.md describes the post-merge state. Read the audit's "★ SANDBOX EXTENSIBILITY STRESS TEST ★" section if you want the rationale for the new APIs.
